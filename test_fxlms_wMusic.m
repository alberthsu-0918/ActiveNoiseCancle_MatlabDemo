...
%         |                                           |    |
%         |                                     ys(k) |    |
%         |        \             (desired signal + noise cancel signal)       
%         |    +-----------+          +-----------+   |    |
%         +--->|   C(z)    |--yw(k)-->|   S(z)    |---+    |
%         |    +-----------+          +-----------+        |
%         |            \                                   |
...
% the noise: 
%X=randn(1,T);
X=randn(1,T)+[zeros(1,9),0.1*music(10:length(music))];

% and measure the arriving noise at the sensor position, plus music from
% canceling speaker
md=filter(Sw, 1, music);               %Error microphone measure the arriving desired signal(for us, it's unkowen)   
...
Yd=filter(Pw, 1, X) + md + mNoise;     %error microphone's measurement
