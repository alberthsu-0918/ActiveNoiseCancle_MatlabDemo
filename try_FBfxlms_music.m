...
% Dh(z) is also modeling by FIR filter.
% 
...
fs = 44.1e3;
music = zeros(1,T);
distort = zeros(1,T);      %unkown, it's used to simulate harmonic distortion 
for t = 1:T
    music(t) = 5*sin(2*pi*f*t/fs);
    distort(t) = 0.25*sin(2*pi*2*f*t/fs);
end
...
Sw=Pw*0.25;
Fw=0.3*[0.03 0.21 0.6 0.9 1.2 0.9 0.6 0.21 0.03]; %F(z) in above figure

% Remember that the first task is to estimate S(z) (and F(z)). So, we can generate a
% white noise signal,
x_iden=randn(1,T);
...
% canceling speaker
md=filter(Sw, 1, music+distort);       %Error microphone measure the arriving desired signal(for us, it's speaker's real output at error microphone and it's unkowen)   
m_est = filter(Shw, 1, music);         %estimation of desired signal at error microphone. 
mNoise = 0.1*randn(1,T);               %music's measuring noise in measure microphone
Yd=filter(Pw, 1, X) + md + mNoise;     %error microphone's actual measurement
...
Sf=zeros(size(Fw));   % the dummy state for the feedback path
...
for k=1:T,                         % discrete time k
    ...
    Sf=[Cy Sf(1:length(Sf)-1)];    % propagate to Feedback path
    e_cont(k)=Yd(k)-sum(Sx.*Sw);   % measure the residue
    e_for_update(k) = e_cont(k) - m_est(k);  %The error we want to minimize is noise, Therefore the error for controller's update has to minus estimation of desired signal
    ...
    Fb = sum(Sf.*Fw);              % update Feedback noise
end
